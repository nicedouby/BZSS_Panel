...
export class WebServer {
  ...
  async handleApi(url, req, res) {
    ...
    const dataManagerHandled = await handleDataManagerRoutes({
      core: this.core,
      modules: this.modules,
      url,
      req,
      ...
      readJsonBody: (request) => this.readJsonBody(request),
      json: (status, obj) => this.json(res, status, obj),
      executeAudited: (context, executor) => this.executeAudited(context, executor),
    });
    if (dataManagerHandled) return;
    ...
    if (url.pathname === "/api/query/playtime-cache" && req.method === "GET") {
      ...
      const squadBrowserBySteamID = new Map(
        squadBrowserTopRows.map((row) => [String(row?.steam_id ?? row?.steamID ?? "").trim(), row]),
      );
        ...
        try {
          ...
          if (row || playerTimeRow) {
            items[steamID] = {
              ...
              squadBrowserBzssMinutes: Number(squadBrowserBySteamID.get(steamID)?.squadbrowser_bzss_minutes ?? 0),
            };
          }
        } catch {}
      }));
    ...
    if (url.pathname === "/api/squad-creation-order/clear" && req.method === "POST") {
      ...
      return this.json(res, 200, {
        ...
        data: {
          serverId,
          lifecycle: this.modules.squadLifecycle?.getCurrent?.(serverId) ?? null,
        },
      });
    }
...
function normalizePlaytimeRow(row, playerTimeRow = null) {
  ...
  return {
    ...
    accountBound: Boolean(playerTimeRow?.qq_number ?? playerTimeRow?.qqNumber),
    qqBoundAt: Number(playerTimeRow?.qq_bound_at ?? playerTimeRow?.qqBoundAt ?? 0) || null,
  };
}
