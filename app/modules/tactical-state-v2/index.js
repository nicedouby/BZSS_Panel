...
export function createTacticalStateV2Module({ core, modules, logger }) {
  const moduleLogger = logger ?? core.createLogger?.({
    moduleId: "module.tacticalStateV2",
    source: "module.tacticalStateV2",
    channel: "module",
  }) ?? core.logger;
  ...
  async function composeSnapshot() {
    ...
    composeInFlight = (async () => {
      ...
      try {
        ...
        const patches = diffSnapshots(oldSnapshot, snapshot);
        const payload = {
          type: "patch",
        ...
        core.eventBus?.emitModuleEvent?.("module.tacticalStateV2", "snapshotUpdated", {
          eventId: `module.tacticalStateV2:${Date.now()}:${state.revision}`,
          eventName: "module.tacticalStateV2.snapshotUpdated",
          layer: "module",
          source: "module.tacticalStateV2",
          serverId,
          time: generatedAt,
  ...
  return {
    manifest: {
      id: "module.tacticalStateV2",
      name: "Tactical State V2 Module",
      kind: "module",
      version: "0.1.0",
...
function diffSnapshots(oldSnap, newSnap) {
  ...
  if (oldSnap && Array.isArray(oldSnap.players)) {
    for (const p of oldSnap.players) {
      if (p?.identity?.key) {
        oldPlayers.set(p.identity.key, p);
      }
    }
  }

  const newPlayers = new Map();
  if (newSnap && Array.isArray(newSnap.players)) {
    for (const p of newSnap.players) {
      if (p?.identity?.key) {
        newPlayers.set(p.identity.key, p);
      }
    }
  }
  ...
  for (const key of oldPlayers.keys()) {
    if (!newPlayers.has(key)) {
      patches.push({ op: "player.remove", key });
    }
  }

  // 2. Assets (fobs, captureZones, mainZones)
  const oldAssets = new Map();
  if (oldSnap && oldSnap.assets) {
    const assetsObj = oldSnap.assets;
    if (Array.isArray(assetsObj.fobs)) {
      for (const f of assetsObj.fobs) {
        if (f?.name) oldAssets.set(`fob:${f.name}`, { type: "fob", ...f });
      }
    }
    if (Array.isArray(assetsObj.captureZones)) {
      for (const z of assetsObj.captureZones) {
        if (z?.name) oldAssets.set(`captureZone:${z.name}`, { type: "captureZone", ...z });
      }
    }
    if (Array.isArray(assetsObj.mainZones)) {
      for (const z of assetsObj.mainZones) {
        if (z?.name) oldAssets.set(`mainZone:${z.name}`, { type: "mainZone", ...z });
      }
    }
  }

  const newAssets = new Map();
  if (newSnap && newSnap.assets) {
    const assetsObj = newSnap.assets;
    if (Array.isArray(assetsObj.fobs)) {
      for (const f of assetsObj.fobs) {
        if (f?.name) newAssets.set(`fob:${f.name}`, { type: "fob", ...f });
      }
    }
    if (Array.isArray(assetsObj.captureZones)) {
      for (const z of assetsObj.captureZones) {
        if (z?.name) newAssets.set(`captureZone:${z.name}`, { type: "captureZone", ...z });
      }
    }
    if (Array.isArray(assetsObj.mainZones)) {
      for (const z of assetsObj.mainZones) {
        if (z?.name) newAssets.set(`mainZone:${z.name}`, { type: "mainZone", ...z });
      }
    }
  }

  for (const [key, asset] of newAssets) {
    const oldAsset = oldAssets.get(key);
    if (!oldAsset || JSON.stringify(oldAsset) !== JSON.stringify(asset)) {
      patches.push({ op: "asset.upsert", key, asset });
    }
  }

  for (const key of oldAssets.keys()) {
    if (!newAssets.has(key)) {
      patches.push({ op: "asset.remove", key });
    }
  }

  // 3. Explosions
  const oldExplosions = new Map();
  if (oldSnap && oldSnap.assets && Array.isArray(oldSnap.assets.explosions)) {
    for (const e of oldSnap.assets.explosions) {
      if (e?.id) oldExplosions.set(e.id, e);
    }
  }

  const newExplosions = new Map();
  if (newSnap && newSnap.assets && Array.isArray(newSnap.assets.explosions)) {
    for (const e of newSnap.assets.explosions) {
      if (e?.id) newExplosions.set(e.id, e);
    }
  }

  for (const [id, explosion] of newExplosions) {
    if (!oldExplosions.has(id)) {
      patches.push({ op: "explosion.add", explosion });
    }
  }

  for (const id of oldExplosions.keys()) {
    if (!newExplosions.has(id)) {
      patches.push({ op: "explosion.remove", id });
    }
  }
