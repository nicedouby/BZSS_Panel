  ...
  <div class="replay-workbench" :class="{ 'is-player-mode': isPlayerMode }">
    <header class="replay-header">
      <div>
        ...
        <p class="subtitle">{{ isPlayerMode ? "独立回放播放器：地图、时间轴和玩家信息均在此处操作。" : "管理录制档案；选择一场对局后进入独立播放器。" }}</p>
      </div>
...
@media (max-width: 1200px) { .is-player-mode .replay-grid { grid-template-columns: minmax(0,1fr) 220px; min-height: auto; } .is-player-mode .inspector { grid-column: auto; min-height: auto; } .inspector-placeholder { min-height: 100px; } .session-overview { grid-template-columns: repeat(3, minmax(0,1fr)); } .archive-summary { grid-template-columns: minmax(220px, 1.5fr) repeat(4, minmax(90px, .6fr)); } .archive-list-heading, .archive-row { grid-template-columns: minmax(220px, 2fr) 110px 82px 82px 78px 102px; gap: 8px; } }
