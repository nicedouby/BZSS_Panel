  ...
  <div class="pressure-settings-page">
    <header class="settings-header">
      <div class="title-block">
        ...
        <div>
          ...
          <h1>压家圈基础参数</h1>
          <p>这里保存全服务器通用参数；单独 Layer 的配置仍可在模拟器中覆盖。</p>
        </div>
      </div>
      <div class="header-actions">
        <RouterLink class="button ghost" to="/debug/pressure-zone">打开模拟器</RouterLink>
    ...
    <template v-else-if="config">
      <section class="preview-strip">
        ...
        <div class="preview-stat hard"><span>Hard 地图基准</span><strong>{{ Math.round(preview.hardMapRadius) }} m</strong></div>
        <div class="preview-stat soft"><span>Soft 地图基准</span><strong>{{ Math.round(preview.softMapRadius) }} m</strong></div>
      ...
      <div class="settings-layout">
        <nav class="section-nav" aria-label="参数分组">
          ...
          <div class="inheritance-note">
            <strong>生效优先级</strong>
            <span>系统默认 → 本页基础参数 → Layer Profile</span>
          </div>
        </nav>
        ...
        <main class="parameter-sections">
          ...
          <section id="hard-policy" class="parameter-card hard-card">
            <header><span>02</span><div><h2>Hard 禁入圈</h2><p>玩家进入后直接判定为压家，半径会同时受地图、最近点和前线距离限制。</p></div></header>
            <div class="parameter-list">
          ...
          <section id="soft-policy" class="parameter-card soft-card">
            <header><span>03</span><div><h2>Soft 警戒圈</h2><p>Hard 圈外的预警范围，允许在战术地图中单独显示和判断。</p></div></header>
            <div class="parameter-list">
...
const preview = computed(() => {
  ...
  return {
    mapScale,
    hardMapRadius: clamp(diagonal * value.hard.mapFactor, value.hard.minRadiusMeters, value.hard.maxRadiusMeters),
    softMapRadius: Math.min(diagonal * value.soft.mapFactor, value.soft.maxRadiusMeters),
...
@media (max-width: 980px) { .settings-header { align-items: flex-start; }.preview-strip { grid-template-columns: repeat(4, 1fr); }.preview-control { grid-column: 1 / -1; }.settings-layout { grid-template-columns: 1fr; }.section-nav { position: static; grid-template-columns: repeat(4, 1fr); }.inheritance-note { display: none; }.section-nav>a { grid-template-columns: 1fr; }.section-nav>a>span,.section-nav small { display: none; } }
@media (max-width: 720px) { .pressure-settings-page { padding: 10px 10px 92px; }.settings-header { flex-direction: column; }.header-actions { width: 100%; justify-content: stretch; }.header-actions>* { flex: 1; }.preview-strip { grid-template-columns: 1fr 1fr; }.preview-control { grid-column: 1 / -1; }.section-nav { grid-template-columns: 1fr 1fr; }.coordinate-mode { grid-template-columns: 1fr; gap: 9px; }.coordinate-actions { flex-wrap: wrap; }.save-bar { right: 10px; bottom: 10px; left: 10px; }.save-bar small { display: none; } }
</style>
