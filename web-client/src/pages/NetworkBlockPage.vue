    ...
    <div class="grid">
      <PageCard title="新增 / 编辑网络阻塞" description="仅支持单个 IPv4 或 IPv6 地址，不支持 IP 段。该 IP 的全部入站连接都会被阻止。必须设置到期时间。">
        <form class="form" @submit.prevent="submit">
          <label><span>选择玩家（自动读取 IP）</span><PlayerSelect v-model="selectedPlayer" placeholder="选择在线玩家或搜索数据库玩家" @select="selectPlayer" /></label><label><span>IP 地址</span><input v-model.trim="draft.ip" placeholder="例如 203.0.113.42" /></label><p v-if="selectedPlayerName" class="selection">已选择：{{ selectedPlayerName }} · {{ draft.ip || "正在读取 IP…" }}</p>
...
const pageError=ref(""); const saving=ref(false); const reconciling=ref(false); const selectingPlayer=ref(false); const editingId=ref(""); const search=ref(""); const selectedPlayer=ref(""); const selectedPlayerName=ref("");
...
function reset(){editingId.value="";selectedPlayer.value="";selectedPlayerName.value="";Object.assign(draft,{ip:"",reason:"",expiresAt:"",durationValue:7,durationUnit:"days"});}
async function selectPlayer(player:any){pageError.value=""; selectingPlayer.value=true; selectedPlayerName.value=String(player?.current_name ?? player?.name ?? "").trim(); try { const response=await apiPost<any>("/api/plugins/network-block/resolve-player-ip",{playerId:player?.id,steamID:player?.steam_id ?? player?.steamID ?? player?.steamId,eosID:player?.eos_id ?? player?.eosID ?? player?.eosId,name:selectedPlayerName.value}); const ip=String(response?.data?.ip ?? "").trim(); if(!ip){pageError.value=`未找到 ${selectedPlayerName.value || "该玩家"} 的当前 IP，无法创建网络阻塞。`;return;} draft.ip=ip; selectedPlayerName.value=String(response?.data?.name ?? selectedPlayerName.value); }catch(error){pageError.value=error instanceof Error?error.message:String(error);}finally{selectingPlayer.value=false;}}
...
<style scoped>.network-block-page{display:grid;gap:16px;min-width:0}.grid{display:grid;grid-template-columns:1.3fr 1fr;gap:16px;min-width:0}.grid>*{min-width:0}.form,label{display:grid;gap:7px}.form{gap:14px}.duration{display:grid;grid-template-columns:1fr 1fr;gap:12px}input,textarea,select,.search{max-width:100%;padding:10px 12px;border:1px solid var(--color-border-default);border-radius:10px;background:var(--color-bg-card);color:var(--color-text-primary)}.table-wrap{max-width:100%;overflow:auto}table{width:100%;min-width:720px;border-collapse:collapse}th,td{padding:11px 9px;border-bottom:1px solid var(--color-border-default);text-align:left}small{display:block;color:var(--color-text-muted);margin-top:3px}.selection{margin:0;color:var(--color-text-muted);font-size:12px}.notice{padding:12px;border:1px solid var(--color-status-danger);border-radius:10px;color:var(--color-status-danger);overflow-wrap:anywhere}@media(max-width:900px){.grid{grid-template-columns:1fr}table{font-size:12px}}@media(max-width:640px){.network-block-page{gap:10px}.duration{grid-template-columns:1fr}.page-header,.panel-head{align-items:stretch;flex-direction:column}.search{width:100%}th,td{padding:8px}}</style>
