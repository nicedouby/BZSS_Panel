...
面向违规小队长的警告不会展示整改倒计时、再次检查时间或自动解散时间。文案统一维护在：

```text
app/modules/squad-restriction-enforcement/messages.js
```
...
| 违规代码 | 警告正文 |
| --- | --- |
| `lock_forbidden` | 当前小队为战斗步兵，战斗步兵禁止锁队，请立即整改。 |
| `solo_lock_forbidden` | 当前小队为载具队，载具队禁止单载，请立即整改。 |
| `locked_player_limit_exceeded` | 当前载具/后勤/迫击炮小队已经超员，请控制小队规模或打开队锁。 |

第一次警告使用“你的小队已经违规”，第二次警告使用“你的小队仍然违规”。后台的 T+30/T+60/T+90 调度不受文案调整影响。
