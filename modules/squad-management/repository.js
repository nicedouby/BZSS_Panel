...
export function createSquadManagementRepository({ config, logger } = {}) {
  ...
  return {
    ...
    async listRecords(query = {}) {
      ...
      if (kind !== "all") {
        if (kind === "action") {
          clauses.push(`(${kindExpr(shape)} = 'disband' OR ${kindExpr(shape)} = 'kick' OR ${kindExpr(shape)} = 'remove' OR ${kindExpr(shape)} = 'ban' OR ${kindExpr(shape)} = 'switch_team' OR ${kindExpr(shape)} = 'action')`);
        } else {
    ...
    async getSummary(query = {}) {
      ...
      const actionTotal = Number(byKind.disband ?? 0) + Number(byKind.kick ?? 0) + Number(byKind.remove ?? 0) + Number(byKind.ban ?? 0) + Number(byKind.switch_team ?? 0) + Number(byKind.action ?? 0);
